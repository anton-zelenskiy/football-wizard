...
def validate_telegram_webapp_data(init_data: str, bot_token: str) -> TelegramWebAppData:
    ...
    try:
        ...
        # Create data string for validation (sorted by key)
        data_check_string = '\n'.join([
            f"{key}={value}" 
...
class RateLimiter:
    ...
    def is_allowed(self, user_id: int, max_requests: int = 100, window: int = 3600) -> bool:
...
def check_rate_limit(user_id: int) -> None:
    """Check if user is within rate limits"""
    if not rate_limiter.is_allowed(user_id, SecurityConfig.RATE_LIMIT_REQUESTS, SecurityConfig.RATE_LIMIT_WINDOW):
        raise HTTPException(
            status_code=429, 
