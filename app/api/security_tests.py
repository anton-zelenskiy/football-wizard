...
def create_mock_telegram_data(
    ...
    # Create user data
    user_data = {'id': user_id, 'first_name': 'Test', 'last_name': 'User', 'username': username}
    ...
    data_check_string = '\n'.join([f'{key}={value}' for key, value in sorted(data_items.items())])
...
def test_validate_telegram_webapp_data_valid():
...
def test_validate_telegram_webapp_data_missing_hash():
...
def test_validate_telegram_webapp_data_expired():
...
def test_validate_telegram_webapp_data_invalid_format():
